...
class StringTest {
    ...
    void should_create_ascii_art() {
        ...
        for (int i = 0; height > i; i++){
            for (int j = 0; width > j; j++){
                if (j == 0 || j == 4){ builder.append("|"); }
                if (i == 0 || i == 2){ if (j > 0 && j < 4){builder.append("-");}
                }else{
                    if (j > 0 && j < 4){ builder.append(" "); }
                }
            }
    ...
    void should_calculate_checksum_of_a_string() {
        ...
        // <--Start
        String[] checkSum = text.split("");
        for (String i : checkSum){
            byte[] bytes = i.getBytes();
            for (byte a : bytes){ sum += a; }
        }
        // --End-->
